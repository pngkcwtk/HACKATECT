...
print(f"✅ TYPHOON_API_KEY: {'พบค่าแล้ว' if TYPHOON_API_KEY else 'ไม่พบค่า'}")
...
class Agent1Output(BaseModel):
    possible_benefits: List[Dict[str, Any]] = Field(
...
class Agent2Output(BaseModel):
    approved_new_benefits: List[Dict[str, Any]] = Field(
        description="รายการสวัสดิการใหม่ที่ผ่านการตรวจสอบแล้วว่าไม่ทับซ้อนและมีสิทธิ์ได้รับ"
    )
    existing_active_benefits: List[Dict[str, Any]] = Field(
...
def call_typhoon_api(
    ...
    """
    ✅ FIX #1: เปลี่ยนจาก .beta.chat.completions.parse เป็น JSON mode + Pydantic parse
    ✅ FIX #6: เพิ่ม Retry mechanism อัตโนมัติ
    ✅ FIX #2: ตรวจสอบ response ก่อน parse เสมอ
    """
    ...
    for attempt in range(1, max_retries + 1):
        try:
            ...
            response = client.chat.completions.create(
                model=MODEL_NAME,
                # ✅ JSON mode ที่รองรับกับ OpenAI-compatible APIs ทั่วไป
                response_format={"type": "json_object"},
                messages=[
                    {"role": "system", "content": system_prompt + "\n\nสำคัญ: ตอบกลับเป็น JSON เท่านั้น ห้ามมี Markdown, backtick, หรือข้อความอื่น"},
                    {"role": "user", "content": user_prompt}
                ],
                temperature=0.1,  # ลด temperature เพื่อความแม่นยำของ structured output
            )

            raw_content = response.choices[0].message.content

            # ✅ FIX #2: ตรวจสอบว่ามี content จริง
            if not raw_content or not raw_content.strip():
                raise ValueError("Model ส่งกลับ response ว่างเปล่า")

            # ✅ ทำความสะอาด response ก่อน parse (กัน edge case)
            clean_content = raw_content.strip()
            if clean_content.startswith("```"):
                clean_content = clean_content.split("```")[1]
                if clean_content.startswith("json"):
                    clean_content = clean_content[4:]
            clean_content = clean_content.strip().rstrip("```").strip()

            # Parse JSON แล้วสร้าง Pydantic model
            ...
            logger.debug(f"  Raw response: {raw_content[:200] if 'raw_content' in locals() else 'N/A'}")
        except Exception as e:
            last_error = e
            logger.warning(f"  ⚠️ เกิดข้อผิดพลาด (attempt {attempt}): {type(e).__name__}: {e}")
    ...
    raise RuntimeError(
        f"API call ล้มเหลวหลังจาก {max_retries} attempts. ข้อผิดพลาดล่าสุด: {last_error}"
    )
...
{json.dumps(agent1_result.possible_benefits, ensure_ascii=False, indent=2)}
...
{json.dumps(agent2_result.existing_active_benefits, ensure_ascii=False, indent=2)}

สิทธิ์ใหม่ที่แนะนำให้ดำเนินการ:
{json.dumps(agent2_result.approved_new_benefits, ensure_ascii=False, indent=2)}
